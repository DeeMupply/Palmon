...
public partial class QuickSceneSwitcher : EditorWindow
{
    ...
    void SaveSettings()
    {
        ...
        EditorPrefs.SetString(GetProjectPrefsKey(PREFS_FOLDERS_KEY), foldersJson);
        EditorPrefs.SetBool(GetProjectPrefsKey(PREFS_SHOW_SETTINGS_KEY), showFolderSettings);
        EditorPrefs.SetFloat(GetProjectPrefsKey(PREFS_FOLDER_HEIGHT_KEY), folderAreaHeight);

        var statesJson = string.Join(";", folderEnabledStates.Select(kvp => $"{kvp.Key}:{kvp.Value}"));
        EditorPrefs.SetString(GetProjectPrefsKey(PREFS_FOLDER_STATES_KEY), statesJson);
    ...
    void LoadSettings()
    {
        string foldersJson = EditorPrefs.GetString(GetProjectPrefsKey(PREFS_FOLDERS_KEY), "");
        ...
        showFolderSettings = EditorPrefs.GetBool(GetProjectPrefsKey(PREFS_SHOW_SETTINGS_KEY), false);
        folderAreaHeight = EditorPrefs.GetFloat(GetProjectPrefsKey(PREFS_FOLDER_HEIGHT_KEY), 200f);

        string statesJson = EditorPrefs.GetString(GetProjectPrefsKey(PREFS_FOLDER_STATES_KEY), "");
